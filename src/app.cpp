...
void app::update(){
    ...
    for(auto &x:_imgs){
        ...
        if(manager::currentTextId!=x->getTexture().id)
            glBindTexture(GL_TEXTURE_2D,x->getTexture().id);
        manager::currentTextId=x->getTexture().id;
        GLint textlocate=x->getShader()->getUniformLocate("Text");
        glUniform1i(textlocate,0);

        GLint camLocate = x->getShader()->getUniformLocate("orthiMat");
        glm::mat4 abc = manager::currentCamera.getCamMatrix();
